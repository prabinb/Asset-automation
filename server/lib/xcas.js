...
CAS.prototype.validate = function(ticket, callback, service, renew)
{
    ...
    //console.log('printing the basepath, validate_path and query: ' + this.base_path+"XXX" + validate_path +"XXX"+ query);
    //console.log('printing the url: ' + this.hostname +this.port+ this.queryPath);
    var req = https.get({
        host: this.hostname,
        port: this.port,
        path: queryPath
...
//		var userAttributes = elemSuccess[0]['cas:attributes'];
...
//		}
                    if (elemSuccess) {
                        ...
                        var userAttributes = elemSuccess['cas:attributes'];
                        callback(undefined, true, username, {
                            'username': username,
